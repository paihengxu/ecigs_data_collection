...
2 Usage Options:
a) from tokenizer import Tokenizer; tokenizer = Tokenizer(**kwargs)
b) from tokenizer import tokenize_raw_text

The former option allows you to specify parameters for tokenization (e.g. 
stopwords, negation, emoji handling, etc.). The latter option uses a default
set of parameters that preserves the content of a piece of text, but removes
some of the typically unnecessary tokens (e.g. Stopwords, numbers, user mentions)
"""
...
## Default Parameters for Social Media Tokenization
TOKENIZER_DEFAULT_PARAMS = dict(
                 stopwords=STOPWORDS,
                 keep_case=False,
                 negate_handling=True,
                 negate_token=False,
...
tokenizer = Tokenizer(**TOKENIZER_DEFAULT_PARAMS)

## Method For Interacting with Default Tokenizer
def tokenize_raw_text(text):
    """
    Wrapper for tokenize method of Tokenizer class with default 
    parameters.
